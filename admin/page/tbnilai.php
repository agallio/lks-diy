<?php
	...
	$sql = "SELECT * FROM nilai INNER JOIN peserta ON nilai.nilai_id_peserta=peserta.peserta_id INNER JOIN lomba ON nilai.nilai_id_lomba=lomba.lomba_id WHERE peserta_nama LIKE '%".$search."%' OR lomba_nama LIKE '%".$search."%' OR nilai_jml LIKE '%".$search."%' ";
		...
		<div class="modal-body">
			...
			<form action="php/cnilai.php" method="POST" class="form" id="fcnilai">
				...
				<select name="lomba" class="form-control" required>
					...
					<?php
						$sql = "SELECT * FROM peserta LEFT JOIN lomba ON peserta.peserta_id_lomba=lomba.lomba_id";
						$query = $mysql->query($sql);
						while ($row = $query->fetch_assoc()) {
					?>
					<option data-val="<?php echo $row['peserta_id_lomba']; ?>" value="<?php echo $row['lomba_id']; ?>"><?php echo $row['lomba_nama']; ?></option>
					<?php } ?>
				</select>
